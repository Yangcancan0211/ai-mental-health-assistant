...
* **📈 个人情绪看板 (ECharts)**：将用户每日的咨询评分转化为 **折线趋势图**，帮助用户直观观察心理好转轨迹。
* **📅 智能情绪日记**：支持基于日历视图的心情记录，AI 自动根据对话生成总结，辅助自我复盘。

### 2. 📊 后台管理端：多维数据赋能 (B-End)

* **🧭 全局情绪大盘 (ECharts)**：
* **雷达图**：分析全平台用户的情绪维度分布（焦虑、平和、积极等）。
* **热力图**：展示咨询高峰时段，辅助心理干预决策。


* **📑 知识库运维**：支持心理科普内容的快速发布、编辑与动态下线。
* **🔐 工业级鉴权**：实现 admin/user 角色权限隔离，采用动态路由与拦截机制保障数据私密性。
